...
"""ValidationService: Centralized validation orchestrator for Acmt001.
...
Example:
    >>> from acmt001.validation import ValidationService, ValidationConfig
    >>>
    >>> config = ValidationConfig(
    ...     xml_message_type="acmt.007.001.05",
    ...     xml_template_file_path="template.xml",
    ...     xsd_schema_file_path="schema.xsd",
    ...     data_file_path="accounts.csv"
    ... )
    >>>
...
from acmt001.xml.validate_via_xsd import validate_via_xsd
...
class ValidationService:
    """Centralized validation orchestrator for Acmt001.
    ...
    def validate_message_type(self, message_type: str) -> ValidationResult:
        ...
        Args:
            message_type: ISO 20022 message type (e.g., 'acmt.007.001.05').
        ...
        Example:
            >>> service = ValidationService()
            >>> result = service.validate_message_type("acmt.007.001.05")
    ...
    def validate_data_source(self, data_path: str | Path) -> ValidationResult:
        ...
        Example:
            >>> service = ValidationService()
            >>> result = service.validate_data_source("accounts.csv")
            >>> if not result.is_valid:
    ...
    def validate_data_content(self, data_path: str | Path) -> ValidationResult:
        ...
        Example:
            >>> service = ValidationService()
            >>> result = service.validate_data_content("accounts.csv")
            >>> if not result.is_valid:
            ...     print(f"Data validation failed: {result.error}")
        """
        try:
            load_account_data(str(data_path))
            return ValidationResult(is_valid=True)
