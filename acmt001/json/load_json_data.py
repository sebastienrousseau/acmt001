...
"""JSON data loader for account data."""
...
def load_json_data(file_path: str) -> list[dict[str, Any]]:
    ...
    Args:
        file_path: Path to JSON file containing account data.
    ...
    Examples:
        ...
        # [{'id': 'MSG001', 'account_currency': 'EUR', ...}, ...]

        # Single object format
        >>> data = load_json_data('account.json')
    ...
    if isinstance(data, dict):
        # Single account object - wrap in list
        return [data]
    elif isinstance(data, list):
        # Array of accounts - validate all are dicts
...
def load_json_data_streaming(
    ...
    Yields:
        Chunks of account data dictionaries.
    ...
    Examples:
        >>> for chunk in load_json_data_streaming('accounts.json', chunk_size=500):
        ...     process_batch(chunk)
    """
...
def load_jsonl_data(file_path: str) -> list[dict[str, Any]]:
    """Load account data from JSON Lines (.jsonl) file.
    ...
    Examples:
        >>> data = load_jsonl_data('accounts.jsonl')
    """
    try:
...
def load_jsonl_data_streaming(
    ...
    Examples:
        >>> for chunk in load_jsonl_data_streaming('large_accounts.jsonl'):
        ...     process_batch(chunk)
    """
    try:
        base_dir = os.getcwd()
