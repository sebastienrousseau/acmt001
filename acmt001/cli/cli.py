...
from acmt001.logging_schema import (
    Events,
    Fields,
    log_event,
    log_validation_event,
)
from acmt001.xml.validate_via_xsd import _get_cached_schema
...
def _validate_schema(
    ...
    """Validate that the XSD schema file is a well-formed ISO 20022 schema.

    Unlike a fully rendered message, the Jinja2 template carries conditional
    blocks and choice branches, so it is validated against the schema only
    after rendering (in ``process_files``). Here we confirm the schema itself
    loads cleanly, surfacing malformed or mismatched XSD files early.
    ...
    Raises:
        SystemExit: If the schema cannot be loaded (exit code 1).
    """
    console.print("[cyan]→ Loading XSD schema...[/cyan]")
    try:
        _get_cached_schema(xsd_schema_file_path)
        log_validation_event(
            logger, "xsd_schema", True, message_type=xml_message_type
        )
        console.print("[bold green]✓ Schema loaded successfully[/bold green]")
    except Exception as e:
        log_validation_event(
            logger, "xsd_schema", False, e, message_type=xml_message_type
        )
        console.print(
            f"[bold red]✗ Schema could not be loaded:[/bold red] {e}",
            style="red",
        )
        console.print(
...
def _validate_account_data(
    ...
    """Validate account data and return record count.

    Args:
        logger: Logger instance for event recording.
        data_file_path: Path to account data file.
        xml_message_type: ISO 20022 message type.

    Returns:
        Number of valid account records.

    Raises:
        SystemExit: If validation fails (exit code 1).
    """
    console.print("[cyan]→ Validating account data...[/cyan]")
    try:
        data = load_account_data(data_file_path)
        record_count = len(data)
        log_validation_event(
            logger, "account_data", True, message_type=xml_message_type
        )
        console.print(
            f"[bold green]✓ Data validation passed[/bold green] "
            f"({record_count} account records)"
        )
        return record_count
    except (FileNotFoundError, ValueError, Exception) as e:
        log_validation_event(
            logger, "account_data", False, e, message_type=xml_message_type
        )
        console.print(
...
def _generate_xml_files(
    ...
    """Generate XML account management files.
    ...
    Args:
        ...
        xml_template_file_path: Path to XML template.
        xsd_schema_file_path: Path to XSD schema.
        data_file_path: Path to account data.
    ...
    """
    console.print("[cyan]→ Generating XML account management files...[/cyan]")
...
@click.command(
    help=(
        "Generate ISO 20022-compliant acmt Account Management "
        "XML files (account opening, maintenance, and closing) from CSV, "
        "SQLite, JSON, or Parquet data.\n\n"
        "EXAMPLES:\n\n"
        "  Basic usage (CSV input):\n"
        "    acmt001 -t acmt.007.001.05 -m template.xml -s schema.xsd -d accounts.csv\n\n"
        "  Validation only (dry-run):\n"
        "    acmt001 -t acmt.007.001.05 -m template.xml -s schema.xsd -d accounts.csv --dry-run\n\n"
        "  Custom output directory:\n"
        "    acmt001 -t acmt.007.001.05 -m template.xml -s schema.xsd -d accounts.csv -o /output\n\n"
        "  Verbose logging:\n"
        "    acmt001 -t acmt.007.001.05 -m template.xml -s schema.xsd -d accounts.csv --verbose\n\n"
        "  JSON input:\n"
        "    acmt001 -t acmt.007.001.05 -m template.xml -s schema.xsd -d accounts.json\n\n"
        "EXIT CODES:\n"
        "  0 = Success\n"
...
@click.option(
    ...
    help="ISO 20022 acmt message type (e.g., 'acmt.007.001.05', 'acmt.019.001.04')",
)
@click.option(
    "-m",
    "--template",
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False, readable=True),
    help="Path to account data file (CSV, SQLite, JSON, JSONL, or Parquet)",
)
@click.option(
    "-c",
    "--config",
...
def main(
    ...
    # pylint: disable=too-many-arguments, too-many-positional-arguments
    """CLI entry point for Acmt001 ISO 20022 acmt Account Management
    file generation.

    Args:
        xml_message_type: ISO 20022 message type (e.g., 'acmt.007.001.05').
    ...
    # Step 7: Confirm the XSD schema loads cleanly
    _validate_schema(
    ...
    if dry_run:
        record_count = _validate_account_data(
