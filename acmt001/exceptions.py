...
"""Custom exception hierarchy for Acmt001.
...
class Acmt001Error(Exception):
    """Base exception for all Acmt001 errors.

    All custom exceptions in this library inherit from this base class,
    allowing consumers to catch any Acmt001-specific error with a single
    ...
    Example:
        ...
        ...     process_files(...)
        ... except Acmt001Error:
        ...     # Catch any Acmt001-specific error
        ...     log.error("Acmt001 operation failed")
    """


class AccountValidationError(Acmt001Error):
...
class XMLGenerationError(Acmt001Error):
...
class ConfigurationError(Acmt001Error):
...
class DataSourceError(Acmt001Error):
...
class SchemaValidationError(Acmt001Error):
