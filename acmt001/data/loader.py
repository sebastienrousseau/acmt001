...
from acmt001.csv.load_csv_data import load_csv_data, load_csv_data_streaming
from acmt001.csv.validate_csv_data import validate_csv_data
...
def _get_file_loaders() -> dict[str, tuple[LoaderFn, ValidatorFn, str]]:
    ...
    return {
        ...
        ".db": (
            lambda p: load_db_data(p, table_name="acmt001"),
            validate_db_data,
            "Database",
        ),
...
def _get_file_stream_loaders() -> (
    ...
    return {
        ...
        ".db": (
            lambda p, cs: load_db_data_streaming(p, "acmt001", cs),
            validate_db_data,
            "Database",
        ),
...
def load_account_data(
    ...
    This function provides a unified interface for loading account data from
    various sources while maintaining backward compatibility with existing
    file-based workflows.

    Args:
        data_source: The account data source. Supports:
            - str: File path to CSV (.csv), SQLite (.db), JSON (.json/.jsonl),
                   or Parquet (.parquet) file
            - list: List of dictionaries with account data
            - dict: Single account instruction as dictionary

    Returns:
        List[Dict[str, Any]]: List of account data dictionaries
    ...
    Examples:
        ...
        >>> data = load_account_data('accounts.csv')
        >>> data = load_account_data('accounts.db')

        # New JSON formats
        >>> data = load_account_data('accounts.json')
        >>> data = load_account_data('accounts.jsonl')  # JSON Lines

        # New Parquet format (requires pyarrow)
        >>> data = load_account_data('accounts.parquet')

        # New direct Python data usage
        >>> data = load_account_data([
        ...     {'msg_id': 'MSG001', 'account_id': 'GB29...', ...},
        ...     {'msg_id': 'MSG002', 'account_id': 'GB30...', ...}
        ... ])
...
def _load_from_file(file_path: str) -> list[dict[str, Any]]:
    ...
    if entry is None:  # pragma: no cover
        # Redundant defensive guard: any extension passing the earlier
        # supported-extension check above has a loader entry here.
        raise DataSourceError(
            f"Unsupported file type: {file_path}. "
            f"Expected .csv, .db, .json, .jsonl, or .parquet file."
        )
...
def _load_from_list(data_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not validate_csv_data(data_list):
        raise AccountValidationError("Data list validation failed")
    return data_list
...
def _load_from_dict(data_dict: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    New feature for single instruction input.
    """
    if not data_dict:
    ...
    data_list = [data_dict]
    if not validate_csv_data(data_list):
        raise AccountValidationError("Data dictionary validation failed")
    return data_list


def load_account_data_streaming(
    ...
    This function yields chunks of account data instead of loading everything
    ...
    Yields:
        List[Dict[str, Any]]: Chunks of account data dictionaries
    ...
    Examples:
        ...
        >>> for chunk in load_account_data_streaming('large_accounts.csv', chunk_size=500):
        ...     process_batch(chunk)

        # Streaming from large SQLite database
        >>> for chunk in load_account_data_streaming('accounts.db', chunk_size=1000):
        ...     generate_xml_batch(chunk)

        # Streaming from large Python list (useful for APIs)
        >>> large_data = [{'msg_id': f'TX{i}', ...} for i in range(100000)]
        >>> for chunk in load_account_data_streaming(large_data, chunk_size=500):
    ...
    Performance:
        ...
        - ~10-15% slower than load_account_data() due to yielding overhead
