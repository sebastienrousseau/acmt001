...
"""Standardized logging schema for Acmt001.
...
class Fields:  # pylint: disable=too-few-public-methods
    ...
    MODULE = "module"
    FUNCTION = "function"
    VERSION = "version"  # Acmt001 library version
    ...
    END_TO_END_ID = "end_to_end_id"  # Instruction reference for tracing
...
def _redact_pii_from_dict(data: dict[str, Any]) -> dict[str, Any]:
    ...
    Example:
        >>> _redact_pii_from_dict({"account_iban": "GB29NWBK60161331926819"})
        {'account_iban': 'GB29****6819'}
    """
...
def configure_json_logging(
    ...
    # pylint: disable=too-many-arguments, too-many-positional-arguments
    """Configure structured JSON logging for Acmt001.
    ...
    Environment Variables:
        ACMT001_LOG_LEVEL: Override log level (DEBUG, INFO, WARNING, ERROR, CRITICAL)
        ACMT001_LOG_FILE: Override log file path
        ACMT001_LOG_JSON: Enable JSON logging (true/false)
    ...
    Example:
        ...
        >>> logger = configure_json_logging(
        ...     log_file="/var/log/acmt001/app.log",
        ...     level=logging.INFO,
    ...
    env_level = os.environ.get("ACMT001_LOG_LEVEL")
    if env_level:
        level = getattr(logging, env_level.upper(), level)

    env_log_file = os.environ.get("ACMT001_LOG_FILE")
    if env_log_file:
