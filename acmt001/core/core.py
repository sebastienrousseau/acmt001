...
from acmt001.security.path_validator import sanitize_for_log, validate_path
...
def _load_data(
    ...
    try:
        account_data = load_account_data(data_file_path)
        ...
        log_event(
            ...
            **{
                Fields.DATA_SOURCE_TYPE: data_source_kind,
                Fields.RECORD_COUNT: len(account_data),
...
def process_files(
    ...
    Args:
        xml_message_type: XML message type (e.g., 'acmt.007.001.05').
    ...
    try:
        ...
        account_data = _load_data(data_file_path, start_time)
        ...
        if os.path.exists(safe_template_path):
            ...
            # Defensive branch: unreachable because generation above wrote
            # ``safe_template_path`` (validated to exist via must_exist=True).
            error_msg = (
