...
1. **Preferred:** GitHub's private vulnerability reporting — open a draft
   advisory at <https://github.com/sebastienrousseau/acmt001/security/advisories/new>.
...
- Code under `acmt001/` shipped to PyPI.
...
If you deploy `acmt001` (CLI, library, or REST API) in production:
...
- Validate paths passed to `process_files` come from a trusted source.
  `acmt001.security.path_validator` is a defence in depth, not a
  substitute for input validation.
- Keep `acmt001`, its runtime dependencies, and the Python interpreter
  patched.
