...
The **acmt001** library generates ISO 20022 acmt Account Management XML
messages (account opening, maintenance, closing, and identification). It ingests
account data from multiple sources (CSV, JSON, JSONL, SQLite, Parquet, Python
...
.. list-table::
   ...
   * - Standard
     - Application
   * - ISO 20022
     - Financial message schema (acmt Account Management message types)
   * - ISO 15022
...
.. list-table::
   ...
   * - 1
     - Core XML Generation
     - Jinja2 template engine, message-type dispatch, XSD validation, namespace
       registration, ``generate_xml_string()`` API
   * - 2
     - Multi-Format Data Ingestion
     - CSV, JSON, JSONL, SQLite, Parquet loaders with streaming support,
       universal ``load_account_data()`` dispatcher
   * - 3
...
3. **Security Scan:** ``bandit -r acmt001/`` reports zero issues
...
.. list-table::
   ...
   * - Deliverable
     - Description
   * - Source Code
     - ``acmt001/`` package with 14 sub-packages, 43 Python modules
   * - Test Suite
