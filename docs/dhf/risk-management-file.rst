...
The acmt001 library is a data transformation tool that generates XML messages
...
.. list-table::
   ...
   * - R-005
     - Invalid BIC, IBAN, or LEI codes in account data produce non-compliant
       messages
     - High
     - Possible
     - High
     - ``validate_bic()`` checks ISO 9362 format rules.
       ``validate_iban()`` verifies ISO 7064 mod-97-10 checksum.
       ``validate_lei()`` verifies ISO 17442 format and check digits. All are
       invoked by ``ValidationService`` before generation.
   * - R-006
     - Non-SWIFT characters in account fields cause message rejection by
       SWIFT network
     - Medium
     - Possible
     - Medium
   ...
   * - R-007
     ...
     - Field length enforcement in ``compliance/swift_charset.py`` checks
       all applicable account fields against SWIFT-defined limits.
   * - R-008
     - Invalid or unsupported acmt message type string causes unexpected
       behavior
     - Medium
     - Unlikely
     - Low
...
.. list-table::
   ...
   * - R-007
     ...
     - Pass
   * - R-008
     - Message-type string allowlist
     - ``test_constants.py``, ``test_message_matrix.py``
     - Pass
   * - R-009
     - Log sanitization
...
.. list-table::
   ...
   * - R-008
     - Low
     - Allowlist is exhaustive for all supported message types
     - Low
     - Yes
   * - R-009
     - Low
