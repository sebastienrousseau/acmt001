...
.. list-table::
   ...
   * - ``test_main_entry.py``
     ...
     - FR-601
   * - ``test_constants.py``
     - ``valid_xml_types`` list, ``message_names`` map, ``BASE_DIR``,
       ``SCHEMAS_DIR``, ``TEMPLATES_DIR``
     - FR-102, NFR-503
   * - ``test_context.py``
...
.. list-table::
   ...
   * - ``test_parquet_loader.py``
     ...
     - FR-205, FR-208
   * - ``test_data_loader.py``
     - Universal ``load_account_data()`` dispatch mechanism
     - FR-201–FR-207
...
.. list-table::
   ...
   * - ``test_lei_validator.py``
     - LEI format + ISO 17442 check-digit verification
     - FR-304
   * - ``test_validation.py``
...
.. list-table::
   ...
   * - bandit
     - SAST
     - ``bandit -r acmt001/`` — zero findings required
   * - safety
     - Dependency audit
...
.. list-table::
   ...
   * - **security**
     - ubuntu-latest, Python 3.12
     - Run ``bandit -r acmt001/`` for SAST scan
