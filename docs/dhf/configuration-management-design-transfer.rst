...
::
   ...
   feat(xml): add acmt.007.001.05 account opening request support
   fix(validation): correct LEI check digit for edge case
   test(gold-master): add acmt.019.001.04 closing reference fixture
...
.. list-table::
   ...
   * - 13
     - mypy
     - Static type checking (strict mode, acmt001/ only)
...
.. list-table::
   ...
   * - Source code
     - Python
     - ``acmt001/`` (43 modules in 14 packages)
   * - Test suite
     - Python
...
This baseline represents the initial release of the acmt001 library with the
...
The acmt001 library is distributed through three channels:
...
- **Build:** ``docker build -t acmt001:latest .``
- **Port:** 8000 (REST API via uvicorn)
- **User:** ``appuser`` (non-root)
- **Health check:** HTTP GET to ``/api/health`` every 30 seconds
- **Startup:** ``python -m uvicorn acmt001.api.app:app --host 0.0.0.0 --port 8000``
...
- **Usage:** Import ``acmt001`` directly or run CLI/REST API
