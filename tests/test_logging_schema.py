"""Tests for the structured logging schema module (acmt001.logging_schema)."""
...
class TestMaskSensitiveData:
    ...
    def test_short_value_fully_masked(self):
        assert mask_sensitive_data("Short", 4) == "****"

    def test_exact_boundary(self):
        assert mask_sensitive_data("12345678", 4) == "****"
...
class TestRedactPiiFromDict:
    def test_redacts_iban_like_account(self):
        result = _redact_pii_from_dict(
            {"account_iban": "GB29NWBK60161331926819"}
        )
        assert "GB29" in result["account_iban"]
        assert "*" in result["account_iban"]

    def test_redacts_bic(self):
        result = _redact_pii_from_dict({"servicer_bic": "DEUTDEFF"})
        assert result["servicer_bic"] == "DEUT**FF"
    ...
    def test_redacts_name(self):
        result = _redact_pii_from_dict({"account_owner_name": "John Doe"})
        assert result["account_owner_name"] == "[REDACTED]"

    def test_redacts_account_number(self):
        result = _redact_pii_from_dict({"account_id": "1234567890ABCDEF"})
        assert result["account_id"] != "1234567890ABCDEF"
        assert result["account_id"].startswith("1234")

    def test_non_pii_unchanged(self):
        result = _redact_pii_from_dict({"msg_id": "MSG-001", "count": 10})
        assert result["msg_id"] == "MSG-001"
        assert result["count"] == 10

    def test_nested_dict_redacted(self):
        result = _redact_pii_from_dict({"owner": {"name": "John Doe"}})
        assert result["owner"]["name"] == "[REDACTED]"

    def test_list_of_dicts_redacted(self):
        result = _redact_pii_from_dict(
            {"accounts": [{"account_owner_name": "Alice"}]}
        )
        assert result["accounts"][0]["account_owner_name"] == "[REDACTED]"
...
class TestLogHelpers:
    def test_log_process_start_returns_timestamp(self):
        ...
        start = log_process_start(logger, "acmt.007.001.05", "csv")
    ...
    def test_log_process_success(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="test_success"):
            log_process_success(logger, start, "acmt.007.001.05", 10)
        assert len(caplog.records) >= 1
        data = json.loads(caplog.records[0].getMessage())
        assert data[Fields.EVENT] == Events.PROCESS_SUCCESS

    def test_log_process_error(self, caplog):
        logger = logging.getLogger("test_error")
        with caplog.at_level(logging.ERROR, logger="test_error"):
            log_process_error(logger, ValueError("test"), "acmt.007.001.05")
        data = json.loads(caplog.records[0].getMessage())
    ...
    def test_log_xml_generation_success(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="test_xml_ok"):
            log_xml_generation_event(
                logger, "acmt.007.001.05", True, record_count=3, duration_ms=50
            )
        data = json.loads(caplog.records[0].getMessage())
    ...
    def test_log_xml_generation_failure(self, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="test_xml_fail"):
            log_xml_generation_event(
                logger, "acmt.007.001.05", False, error=RuntimeError("xsd")
            )
        data = json.loads(caplog.records[0].getMessage())
    ...
    def test_log_xml_generation_failure_no_error(self, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="test_xml_fail2"):
            log_xml_generation_event(logger, "acmt.007.001.05", False)
        data = json.loads(caplog.records[0].getMessage())
        assert data["error_type"] == "Unknown"
    ...
    def test_dry_run_mode(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="test_dry"):
            tracker = ExecutionSummaryTracker(
                logger, dry_run=True, message_type="acmt.007.001.05"
            )
            tracker.start()
            tracker.log_summary()
...
class TestConfigureJsonLogging:
    ...
    def test_console_handler_added(self):
        logger = configure_json_logging(
            logging.getLogger("test_console"), console_output=True
        )
    ...
    def test_env_var_override(self, monkeypatch):
        monkeypatch.setenv("ACMT001_LOG_LEVEL", "DEBUG")
        logger = configure_json_logging(
    ...
    def test_env_var_log_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ACMT001_LOG_FILE", log_file)
        logger = configure_json_logging(
    ...
    def test_defaults_to_root_logger(self):
        logger = configure_json_logging(console_output=False)
...
class TestExecutionMetrics:
    def test_basic_lifecycle(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="test_metrics"):
            metrics = ExecutionMetrics(
                logger, "xml_generation", message_type="acmt.007.001.05"
            )
            metrics.start()
    ...
    def test_telemetry_failed_status_logs_error(self, caplog):
        logger = logging.getLogger("test_metrics_err_log")
        with caplog.at_level(logging.ERROR, logger="test_metrics_err_log"):
            metrics = ExecutionMetrics(logger, "test")
            metrics.start()
            metrics.set_error("boom")
            metrics.log_telemetry()
