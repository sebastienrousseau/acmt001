"""Tests for acmt001.api.job_manager."""

from acmt001.api.job_manager import JobManager, JobResult, JobStatus
...
class TestJobManager:
    ...
    def test_update_with_error(self):
        mgr = JobManager()
        job_id = mgr.create_job()
        mgr.update_status(job_id, JobStatus.FAILED, error="broke")
        assert mgr.get_job(job_id).error == "broke"

    def test_update_nonexistent_job(self):
        JobManager().update_status("nope", JobStatus.FAILED)  # no raise
    ...
    def test_cancel_completed_job(self):
        ...
        assert not mgr.cancel_job(job_id)

    def test_cancel_nonexistent_job(self):
        assert not JobManager().cancel_job("nonexistent")
    ...
    def test_cleanup_under_keep_count_noop(self):
        mgr = JobManager()
        jid = mgr.create_job()
        mgr.update_status(jid, JobStatus.SUCCESS)
        mgr.cleanup_old_jobs(keep_count=100)
        assert jid in mgr.jobs
