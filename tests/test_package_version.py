...
how `0.0.3` reached PyPI with `cryptography<50.0.0` after the tree had
...
def test_dunder_version_is_semver() -> None:
    assert SEMVER.match(
        acmt001.__version__
    ), f"__version__ is {acmt001.__version__!r}, which is not X.Y.Z"


def test_dunder_version_matches_pyproject() -> None:
    assert acmt001.__version__ == _pyproject_version(), (
        f"acmt001.__version__ is {acmt001.__version__!r} but "
...
def test_changelog_documents_the_current_version() -> None:
    ...
    assert acmt001.__version__ in versions, (
        f"CHANGELOG.md has no entry for {acmt001.__version__}; "
...
def test_changelog_newest_entry_is_the_current_version() -> None:
    versions = _changelog_versions()
    assert versions[0] == acmt001.__version__, (
        f"the newest CHANGELOG entry is {versions[0]} but the package is "
        f"{acmt001.__version__} — a release was cut without a changelog "
...
def test_constants_version_matches_dunder_version() -> None:
    """`constants.VERSION` is a fifth restatement of the same number."""
    from acmt001.constants import VERSION

    assert VERSION == acmt001.__version__, (
        f"acmt001.constants.VERSION is {VERSION!r} but __version__ is "
        f"{acmt001.__version__!r}"
    )
