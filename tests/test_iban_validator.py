...
class TestValidateIbanFormat:
    ...
    def test_wrong_country_length(self):
        # DE IBAN should be 22 chars; give a too-short-for-DE but valid range
        ok, err = validate_iban_format("DE8937040044053201300")
        assert not ok
...
class TestValidateIbanChecksum:
    ...
    def test_invalid_character_in_checksum(self):
        # '!' is not alphanumeric -> rejected by the per-char branch
        ok, err = validate_iban_checksum("DE89370400440532013!00")
        assert not ok
...
class TestValidateIban:
    ...
    def test_invalid_format_strict_raises(self):
        with pytest.raises(InvalidIBANError) as exc_info:
            validate_iban("", field="account_id")
...
class TestValidateIbanSafe:
    ...
    def test_with_field_name(self):
        assert validate_iban_safe("GB29NWBK60161331926819", field="account")
