...
def csv_file(tmp_path, monkeypatch):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["msg_id", "account_currency", "account_name"]
        )
        writer.writeheader()
        writer.writerow(
            {
...
def large_csv(tmp_path, monkeypatch):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["msg_id", "account_currency"])
        writer.writeheader()
        for i in range(50):
            writer.writerow(
                {"msg_id": f"ACMT-{i:04d}", "account_currency": "EUR"}
            )
    return str(path)
...
class TestLoadCsvDataStreaming:
    ...
    def test_path_traversal_raises(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with pytest.raises(Exception):
            list(load_csv_data_streaming("../../etc/passwd"))
