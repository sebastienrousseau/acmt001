...
def json_array_file(tmp_path, monkeypatch):
    """Create a JSON file with an array of account data."""
    monkeypatch.chdir(tmp_path)
    data = [
        {"msg_id": "ACMT-001", "account_currency": "EUR"},
        {"msg_id": "ACMT-002", "account_currency": "USD"},
    ]
...
def json_single_file(tmp_path, monkeypatch):
    """Create a JSON file with a single account object."""
    monkeypatch.chdir(tmp_path)
    data = {"msg_id": "ACMT-SINGLE", "account_currency": "GBP"}
...
def jsonl_file(tmp_path, monkeypatch):
    """Create a JSONL file with multiple lines (incl. a blank one)."""
    monkeypatch.chdir(tmp_path)
    path = tmp_path / "accounts.jsonl"
    lines = [
        json.dumps({"msg_id": "ACMT-L1", "account_currency": "EUR"}),
        "",  # empty line, should be skipped
        json.dumps({"msg_id": "ACMT-L2", "account_currency": "USD"}),
    ]
...
class TestLoadJsonlData:
    ...
    def test_load_jsonl(self, jsonl_file):
        ...
        assert len(data) == 2
        assert data[0]["msg_id"] == "ACMT-L1"

    def test_skips_empty_lines(self, jsonl_file):
        data = load_jsonl_data(jsonl_file)
        assert len(data) == 2  # blank line skipped
